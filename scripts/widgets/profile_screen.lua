...
local profile_screen = Class(Widget, function(self, owner)
    ...
    for k, v in pairs(self.owner.genshin_profile_addition) do
        ...
        -- local title = Text("genshinfont", 28, titlestr, {246/255, 242/255, 238/255, 1})
        -- self.detail_list:AddItem(title)
        -- title:SetHAlign(ANCHOR_LEFT)
        -- title:SetVAlign(ANCHOR_MIDDLE)
        -- title:SetRegionSize(400, 60)
        -- title:EnableWordWrap(true)
        -- title:SetPosition(40, y, 0)
        -- local valwidget = Text("genshinfont", 28, valuestr, {246/255, 242/255, 238/255, 1})
        -- self.detail_list:AddItem(valwidget)
	    -- valwidget:SetHAlign(ANCHOR_RIGHT)
        -- valwidget:SetVAlign(ANCHOR_MIDDLE)
        -- valwidget:SetRegionSize(400, 60)
        -- valwidget:EnableWordWrap(true)
	    -- valwidget:SetPosition(-40, y, 0)
