...
local GIngredientUI = require "widgets/genshin_widgets/Gingredientui"
...
-------------------------------------------------------------------------------------------------------
local GCraftingMenuIngredients = Class(GWidget, function(self, owner, max_ingredients_wide, recipe, extra_quantity_scale)
    GWidget._ctor(self, "CraftingMenuIngredients")
...
function GCraftingMenuIngredients:SetRecipe(recipe)
    ...
    self.ingredient_widgets = {}
	local root = self:AddChild(GWidget("root"))
    ...
    for i, v in ipairs(recipe.tech_ingredients) do
        if v.type:sub(-9) == "_material" then
            ...
            local ing = root:AddChild(GIngredientUI(v:GetAtlas(), v:GetImage(), nil, nil, has, STRINGS.NAMES[string.upper(v.type)], owner, v.type, quant_text_scale))
    ...
    for i, v in ipairs(recipe.ingredients) do
        ...
        local ing = root:AddChild(GIngredientUI(v:GetAtlas(), v:GetImage(), v.amount ~= 0 and v.amount or nil, num_found, has, STRINGS.NAMES[string.upper(v.type)], owner, v.type, quant_text_scale, ingredient_recipe_data))
        if GetGameModeProperty("icons_use_cc") then
            ing.ing:SetEffect("shaders/ui_cc.ksh")
        end
        if num > 1 and #self.ingredient_widgets > 0 then
    ...
    for i, v in ipairs(recipe.character_ingredients) do
        ...
        local ing = root:AddChild(GIngredientUI(v:GetAtlas(), v:GetImage(), v.amount, amount, has, STRINGS.NAMES[string.upper(v.type)], owner, v.type, quant_text_scale))
        if GetGameModeProperty("icons_use_cc") then
            ing.ing:SetEffect("shaders/ui_cc.ksh")
        end
        if num > 1 and #self.ingredient_widgets > 0 then
	...
	if showamulet then
		...
		local amulet = root:AddChild(GIngredientUI(amulet_atlas, amulet_img, 0.2, 0.2, true, STRINGS.GREENAMULET_TOOLTIP, owner, CHARACTER_INGREDIENT.MAX_HEALTH, quant_text_scale))
        ...
        for _, ing in ipairs(self.ingredient_widgets) do
			local glow = ing:AddChild(GImage("images/global_redux.xml", "shop_glow.tex"))
...
return GCraftingMenuIngredients
