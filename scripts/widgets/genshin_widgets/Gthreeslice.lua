...
local GThreeSlice = Class(GWidget, function(self, atlas, cap, filler, end_cap)
    GWidget._ctor(self, "ThreeSlice")
	...
	self.root = self:AddChild(GWidget("root"))
    self.startcap = self.root:AddChild(GImage(atlas, cap))
    self.endcap = self.root:AddChild(GImage(atlas, end_cap))
    self.parts = {}
end)


function GThreeSlice:SetImages(atlas, cap, filler, end_cap)
    self.atlas = atlas
    self.filler = filler
...
function GThreeSlice:RemoveParts()
	for k,v in pairs(self.parts) do
		v:Kill()
	end
	self.parts = {}
...
function GThreeSlice:Flow(width, height, horizontal)
	self:RemoveParts()
	...
	if fill_dist > 0 then
		...
		local filler = self.root:AddChild(GImage(self.atlas, self.filler))
		...
		for k = 1, num_filler do
			if filler == nil then
				filler = self.root:AddChild(GImage(self.atlas, self.filler))
			end
...
function GThreeSlice:ManualFlow(num_filler, horizontal)
	self:RemoveParts()
