...
local GenshinRechargeable = Class(function(self, inst)
    self.inst = inst
    self.recharge = -2
	...
	self.startrechargefn = nil
	self.stoprechargefn = nil
end)

function GenshinRechargeable:SetRechargeTime(time)
	self.rechargetime = time  --totaltime
	self.recharge = 0
end

function GenshinRechargeable:SetRechargeRate(fn)
	self.rechargingrate = fn
end

function GenshinRechargeable:StartRecharging()
	...
	if self.startrechargefn ~= nil then
		self.startrechargefn(self.inst)
	end
...
function GenshinRechargeable:StopRecharging()
	...
	if self.stoprechargefn ~= nil then
		self.stoprechargefn(self.inst)
	end
...
function GenshinRechargeable:OnUpdate(dt)
...
function GenshinRechargeable:GetPercent()
...
function GenshinRechargeable:GetDebugString()
...
function GenshinRechargeable:GetRechargeTime()
	return self.recharging == true  and (self.target_time ~= nil and self.target_time or self.rechargetime) or  0
end

function GenshinRechargeable:LeftTimeDelta(time)
    self.inst:StopUpdatingComponent(self)
	if not self.recharging then
	    return
	end
	self.recharge = time + self.recharge
	self.inst:PushEvent("rechargechange", {percent = self:GetPercent()} )
	self.inst:StartUpdatingComponent(self)
end

function GenshinRechargeable:OnSave()
    return
    {
        time = self.recharge,
...
function GenshinRechargeable:OnLoad(data)
